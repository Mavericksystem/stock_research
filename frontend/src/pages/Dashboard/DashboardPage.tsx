...
function EventBadge({
  ...
  return (
    <span className={`rounded px-2 py-[3px] font-mono text-[11px] font-medium tracking-[0.5px] ${isSpike ? "border border-emerald-400/25 bg-emerald-400/10 text-[var(--green)]" : isDrop ? "border border-red-400/25 bg-red-400/10 text-[var(--red)]" : "border border-[var(--amber-border)] bg-[var(--amber-dim)] text-[var(--amber)]"}`}>
      {label}
      {mag}
    </span>
  );
...
function SourceCard({ source }: { source: Source }) {
  const score = source.relevance_score != null ? source.relevance_score.toFixed(2) : "—";

  const card = (
    <motion.div whileHover={{ x: 2 }} transition={{ duration: 0.15 }} className="flex items-start gap-2.5 rounded-[var(--radius)] border border-[var(--border-soft)] bg-[var(--surface)] px-3 py-2.5 transition-colors hover:border-[var(--amber-border)]">
...
function AssistantMessage({ msg }: { msg: Extract<Message, { type: "assistant" }> }) {
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.28, ease: "easeOut" }} className="flex w-full flex-col gap-3">
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-2.5">
      ...
      <div className="flex gap-1 pl-1">
        {[0, 1, 2].map((index) => (
          <motion.span key={index} animate={{ opacity: [0.2, 1, 0.2], y: [0, -2, 0] }} transition={{ duration: 1.2, repeat: Infinity, delay: index * 0.2, ease: "easeInOut" }} className="h-[5px] w-[5px] rounded-full bg-[var(--text-muted)]" />
        ))}
      </div>
    </motion.div>
  );
    ...
    <div className="flex flex-col gap-2 pl-[34px]">
      ...
      <div className="flex flex-col gap-1.5">
        <AnimatePresence initial={false}>
          {steps.map((step) => (
            <motion.div key={step.id} initial={{ opacity: 0, x: -4 }} animate={{ opacity: step.status === "done" ? 0 : 1, x: 0 }} exit={{ opacity: 0, x: 4 }} transition={{ duration: 0.2 }} className={`flex items-center gap-2 font-mono text-[11px] ${step.status === "done" ? "text-[var(--green)]" : "text-[var(--text-muted)]"}`}>
              <motion.span animate={step.status === "running" ? { rotate: 360 } : { rotate: 0 }} transition={step.status === "running" ? { duration: 0.9, repeat: Infinity, ease: "linear" } : { duration: 0.15 }} className={`h-2.5 w-2.5 shrink-0 rounded-full border ${step.status === "running" ? "border-[var(--amber-border)] border-t-[var(--amber)]" : "border-[var(--green)] bg-[var(--green)]"}`} />
...
function EmptyState({
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease: "easeOut" }} className="m-auto text-center">
      ...
      <div className="mb-4 flex flex-wrap justify-center gap-2">
        {suggestions.map((suggestion) => (
          <motion.button key={suggestion} type="button" whileHover={{ y: -1 }} whileTap={{ scale: 0.97 }} onClick={() => onSuggestion(suggestion)} className="suggestion-card">
            <span className="suggestion-size">{suggestion}</span>
            <span className="suggestion-border" />
            <p className="suggestion-text" data-title={suggestion} data-text="Know more" />
          </motion.button>
        ))}
      </div>
...
function SearchIcon() {
  return (
    <svg className="search-icon" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <path d="M24 4L27.8 16.2L40 20L27.8 23.8L24 36L20.2 23.8L8 20L20.2 16.2L24 4Z" />
      <path d="M8 5L9.5 9.5L14 11L9.5 12.5L8 17L6.5 12.5L2 11L6.5 9.5L8 5Z" />
      <path d="M24 27L25.2 30.8L29 32L25.2 33.2L24 37L22.8 33.2L19 32L22.8 30.8L24 27Z" />
    </svg>
  );
}
    ...
    <>
      <div className="flex min-h-0 flex-1 flex-col items-center gap-6 overflow-y-auto px-0 pb-2 pt-6">
        {messages.length === 0 && !loading ? (
          <EmptyState onSuggestion={onSuggestion} onStock={onStock} suggestions={suggestions} trackedStocks={trackedStocks} />
        ) : (
          <div className="content-width flex flex-col gap-6">
            {messages.map((msg) => (
              <div key={msg.id} className="w-full">
                {msg.type === "user" ? (
                  <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }} className="flex justify-start">
